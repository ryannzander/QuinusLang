...
fn test_tokenize_integers() {
    ...
    assert!(matches!(t, qpp::lexer::Token::Int(42)));
}
...
fn test_tokenize_digit_separators() {
    ...
    let (t, _, _) = stream.consume().unwrap();
    assert!(matches!(t, qpp::lexer::Token::Int(1000000)));
}
...
fn test_tokenize_identifiers() {
    ...
    let (t, _, _) = stream.consume().unwrap();
    assert!(matches!(t, qpp::lexer::Token::Ident(s) if s == "foo"));
    let (t, _, _) = stream.consume().unwrap();
    assert!(matches!(t, qpp::lexer::Token::Ident(s) if s == "bar_baz"));
}
